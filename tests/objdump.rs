...
#[test]
fn test_objdump() {
    let base_paths = vec![Path::new("tests/assets"), Path::new("tests/assets/wast")];
    ...
    for base in base_paths {
        ...
        for entry in fs::read_dir(base).expect("Should be able to read the folder content") {
            ...
            if file.extension().and_then(|ext| ext.to_str()) == Some("wasm") {
                ...
                // call aarch64-linux-gnu-objdump -d -s -t $file > ${file%.o}.asm
                let object_file = std::process::Command::new("aarch64-linux-gnu-objdump")
                    .args(&["-d", "-s", "-t", output_path.to_str().unwrap()])
                    .output()
                    .expect("Failed to execute objdump");
                let asm_output = String::from_utf8_lossy(&object_file.stdout);
                let asm_output_path = output_path.with_extension("asm");
                fs::write(&asm_output_path, asm_output.as_ref())
                    .expect("fs::write() should be able to write");

                // remove the .o file after dumping
                fs::remove_file(&output_path).expect("Should be able to remove the file");
            }
        }
    }
}
